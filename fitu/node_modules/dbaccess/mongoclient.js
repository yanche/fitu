...
dbClient.mapReduce = function (collName, mapFn, reduceFn, query) {
    var defer = new bluebird.defer();
    dbClient.getColl(collName).mapReduce(mapFn, reduceFn, {query: query || {}, out: { inline: 1 }}, function (err, docs) {
        if (err)
            defer.reject(err);
        else
            defer.resolve(docs);
