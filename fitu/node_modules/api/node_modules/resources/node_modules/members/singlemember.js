...
SingleMember.prototype._accessibility = function () {
    ...
    if (!me.filter.memberObjId)
        ...
        .then(function (data) {
            ...
            if (!mem)
                return httpOptions.setStatus('GET', 404).setStatus('POST', 404).setStatus('DELETE', 404);
            else if (god)
                return httpOptions.setStatus('GET', 200).setStatus('POST', 200).setStatus('DELETE', 200);
            else {
                httpOptions.setStatus('GET', 401).setStatus('POST', 401).setStatus('DELETE', 401);
                if (mem.userId.equals(me.operator.objId)) //self
                    httpOptions.setStatus('GET', 200).setStatus('DELETE', 200);
                if (act && act.createdBy.equals(me.operator.objId))
                    httpOptions.setStatus('GET', 200).setStatus('POST', 200);
                return httpOptions;
            }
        });
    }
...
SingleMember.prototype._post = function () {
    var me = this;
    return me._accessibility()
    .then(function (httpOptions) {
    ...
    .then(function (currentMem) {
        ...
        //payStatusId
        if (validate.value(me.args.payStatusId)) {
            if (me.args.payStatusId == constants.status.memberPayStatus.paid || me.args.payStatusId == constants.status.memberPayStatus.unpaid)
                update.payStatusId = me.args.payStatusId;
            else
                throw new errdef.DeferChainKiller(extension.http.webres400());
        }
