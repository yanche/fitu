...
var SingleMember = function (webreq) {
    ...
    me.writeAsUser = false;
    me.writeAsOrganizer = false;
};
    ...
    if (!me.filter.memberObjId)
        ...
        .then(function (data) {
            ...
            if (!mem)
                ...
                me.writeAsUser = true;
                me.writeAsOrganizer = true;
                return httpOptions.setStatus('GET', 200).setStatus('PUT', 200).setStatus('DELETE', 200);
            }
            else {
                httpOptions.setStatus('DELETE', 401);
                if (mem.userId.equals(me.operator.objId))
                    me.writeAsUser = true;
                if (act && act.createdBy.equals(me.operator.objId))
                    me.writeAsOrganizer = true;
                if (me.writeAsUser || me.writeAsOrganizer)
                    return httpOptions.setStatus('GET', 200).setStatus('PUT', 200);
                else
                    return httpOptions.setStatus('GET', 401).setStatus('PUT', 401);
            }
        });
    }
};
...
SingleMember.prototype._get = function () {
    ...
    .then(function (httpOptions) {
        ...
        if (status.isOK())
            return bluebird.all([me.targetMember_CL.load(), me.targetUser_CL.load(), me.targetActivity_CL.load()]);
        else
            throw new errdef.DeferChainKiller(status.toWebres());
    })
    .then(function (data) {
        var mem = data[0], user = data[1], act = data[2];
        if (mem)
            return resxutil.member.convMemberInfo(member, user ? [user] : [], act ? [act] : []);
        else
...
SingleMember.prototype._put = function () {
    ...
    .then(function (currentMem) {
        ...
        if (validate.value(me.args.statusId)) {
            if (me.args.statusId == constants.status.memberStatus.queued || me.args.statusId == constants.status.memberStatus.confirmed) {
                if (me.writeAsOrganizer)
                    update.statusId = me.args.statusId;
                else
                    throw new errdef.DeferChainKiller(extension.http.webres401());
            }
            else if (me.args.statusId == constants.status.memberStatus.quit) {
                if (me.writeAsUser)
                    update.statusId = me.args.statusId;
                else
                    throw new errdef.DeferChainKiller(extension.http.webres401());
