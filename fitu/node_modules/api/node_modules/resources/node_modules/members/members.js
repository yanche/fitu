...
Members.prototype._get = function () {
    ...
    .then(function (httpOptions) {
        ...
        if (status.isOK()) {
            return bluebird.all([
                db.member.getMembersFieldsInPage({ actId: me.filter.actObjId }, resxutil.member.getFields, { createdOn: -1 }, me.page * me.pageSize, me.pageSize),
                db.member.countMembers({ actId: me.filter.actObjId })
            ]);
        }
        else
...
Members.prototype._post = function () {
    ...
    .then(function (httpOptions) {
        ...
        if (status.isOK()) {
            var create = { createdOn: new Date(), actId: me.filter.actObjId, userId: me.operator.objId, statusId: constants.status.memberStatus.active };
            return bluebird.all([db.member.getOneMemberFieldsBy({ actId: create.actId, userId: create.userId, statusId: constants.status.memberStatus.active }, { _id: 1 }), db.activity.getOneActFieldsBy(create.actId, { _id: 1, name: 1, createdBy: 1, startsOn: 1, statusId: 1 })])
            .then(function (data) {
                if (data[0])
    ...
    .then(function (inserted) {
        ...
        note.toSysNote({ emphasis: false, subject: '活动**' + targetAct.name + '**报名成功啦！', subjectType: constants.note.contentTypes.markdown, body: '活动详情请[查看](/actdetail?actId=' + targetAct._id.toString() + ')', bodyType: constants.note.contentTypes.markdown, recipients: [me.operator.objId] });
        //TODO: log if failed
        ...
        //TODO: improve the format
        note2.toSysNote({ emphasis: false, subject: '你组织的活动*' + targetAct.name + '*有人报名啦！', subjectType: constants.note.contentTypes.markdown, body: '已报名人员的名单请[查看](/myself/actlead?actId=' + targetAct._id.toString() + ')', bodyType: constants.note.contentTypes.markdown, recipients: [targetAct.createdBy] });
        //TODO: log if failed
        db.note.insertOneNote(note2);
