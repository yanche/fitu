...
exports.getFields = { '_id': 1, 'personal': 1, 'email': 1, 'headUrl': 1, 'fans': 1, 'subscribe': 1, 'mobileV': 1, 'special': 1, 'hash_pwd': 1, 'wechatOpenId': 1 };
exports.convUserInfo = function (user) {
    return { 'id': user._id.toString(), 'personal': user.personal, 'email': user.email, 'headUrl': user.headUrl, 'fansCount': user.fans.length, 'subscribe': { usersCount: user.subscribe.users.length, sitesCount: user.subscribe.sites.length }, 'mobileV': user.mobileV && user.personal && user.personal.phone && user.mobileV.phone == user.personal.phone, special: user.special, 'wechatInit': !user.hash_pwd && user.wechatOpenId };
};
