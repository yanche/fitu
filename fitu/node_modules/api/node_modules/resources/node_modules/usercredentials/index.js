...
SingleCredential.prototype._accessibility = function () {
    ...
    if (!me.filter.userObjId)
        return bluebird.resolve(httpOptions.setStatus('PUT', 400));
    else if (!me.operator)
        return bluebird.resolve(httpOptions.setStatus('PUT', 401));
    else if (me.filter.userObjId.equals(me.operator.objId))
        return bluebird.resolve(httpOptions.setStatus('PUT', 200));
    else {
        return me.operator.isGod()
        .then(function (ok) {
            if (ok)
                me.confirmPwd = false;
            return httpOptions.setStatus('PUT', ok ? 200 : 401);
        });
    }
};

SingleCredential.prototype._put = function () {
    var me = this;
    ...
    .then(function (current) {
        ...
        else if (me.confirmPwd && current.hash_pwd != (me.args.confirm_hash_pwd || '').toLowerCase()) //origin pwd not match
        ...
        return resxutil.user.checkDup(update.email)
        .then(function (ok) {
            if (ok)
