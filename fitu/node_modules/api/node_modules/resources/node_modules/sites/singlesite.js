...
SingleSite.prototype._accessibility = function () {
    ...
    if (me.operator) {
        ...
        .then(function (data) {
            ...
            if(!site)
                return httpOptions.setStatus('GET', 404).setStatus('POST', 404).setStatus('DELETE', 404); //site not found
            else if (god)
                ...
                if (site.vendorId) {
                    ...
                    .then(function (ok) {
                        return httpOptions.setStatus('POST', ok ? 200 : 401).setStatus('DELETE', ok ? 200 : 401);
                    });
                }
                else
                    return httpOptions.setStatus('POST', 200).setStatus('DELETE', 401);
            }
        });
    }
    else
...
SingleSite.prototype._get = function () {
    ...
    .then(function (st) {
        if (st) {
            site = st;
            return st.vendorId ? db.vendor.getOneVendorFieldsBy({ _id: st.vendorId }, resxutil.vendor.getRelatedVendorFields) : null;
        }
        else
...
SingleSite.prototype._post = function () {
    var me = this;
    return me._accessibility()
    .then(function (httpOptions) {
        ...
        if (validate.value(me.args.trans)) {
            if (validate.value(me.args.trans.subway)) {
                if (validate.isString(me.args.trans.subway))
                    update['trans.subway'] = me.args.trans.subway.trim();
                else
                    throw new errdef.DeferChainKiller(extension.http.webres400());
            }
            if (validate.value(me.args.trans.bus)) {
                if (validate.isString(me.args.trans.bus))
                    update['trans.bus'] = me.args.trans.bus.trim();
                else
                    throw new errdef.DeferChainKiller(extension.http.webres400());
            }
        }
...
SingleSite.prototype._delete = function () {
    ...
    .then(function (currentSt) {
        if (currentSt)
            return db.site.updateSites({ _id: me.filter.siteObjId }, { $set: { statusId: constants.status.siteStatus.inactive } });
        else
            throw new errdef.DeferChainKiller(extension.http.webres404());
    })
    .then(function () {
