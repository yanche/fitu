...
Activities.prototype._get = function () {
    ...
    .then(function (httpOptions) {
        ...
        if (status.isOK()) {
            ...
            if (me.filter.active) {
                filter.statusId = constants.status.actStatus.active;
    ...
    .then(function (data) {
        ...
        return bluebird.all([
            ...
            db.member.getMembersFieldsBy({ actId: { $in: activities.map(function (a) { return a._id; }) }, statusId: constants.status.memberStatus.active }, { _id: 1, actId: 1 }),
...
Activities.prototype._post = function () {
    ...
    .then(function (inserted) {
        ...
        note.toSysNote({ emphasis: false, subject: '活动**' + inserted.name + '**创建成功！', subjectType: constants.note.contentTypes.markdown, body: '活动详情请[查看](/actdetail?actId=' + inserted._id.toString() + ')  \n已报名人员名单请[查看](/myself/actlead?actId=' + inserted._id.toString() + ')  \n快召集小伙伴一起动起来！', bodyType: constants.note.contentTypes.markdown, recipients: [inserted.createdBy] });
        //TODO: log if failed
        db.note.insertOneNote(note);
        
        //note to site fans
        if (validate.nonEmptyArray(targetSite.fans)) {
            var note = new resxutil.note.Note();
            //TODO: improve the format
            note.toSysNote({ emphasis: false, subject: '有人在你所关注的场馆*' + targetSite.name + '*召集活动了哟！', subjectType: constants.note.contentTypes.markdown, body: '活动名称：' + inserted.name + '  \n详情请[查看](/actdetail?actId=' + inserted._id.toString() + ')  \n一起动起来！', bodyType: constants.note.contentTypes.markdown, recipients: targetSite.fans });
            //TODO: log if failed
            db.note.insertOneNote(note);
        }
        ...
        .then(function (myinfo) {
            if (validate.nonEmptyArray(myinfo.fans)) {
                var note = new resxutil.note.Note();
                //TODO: improve the format
                note.toSysNote({ emphasis: false, subject: '你关注的*' + (myinfo.personal.nickName || '某匿名同学') + '*召集活动了哟！', subjectType: constants.note.contentTypes.markdown, body: '活动名称：' + inserted.name + '  \n详情请[查看](/actdetail?actId=' + inserted._id.toString() + ')  \n快来跟TA一起动起来！', bodyType: constants.note.contentTypes.markdown, recipients: myinfo.fans });
                //TODO: log if failed
                db.note.insertOneNote(note);
            }
        });
