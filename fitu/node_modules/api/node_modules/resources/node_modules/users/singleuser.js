...
SingleUser.prototype._accessibility = function () {
    ...
    if (!me.filter.userObjId)
        ...
        return bluebird.resolve(httpOptions.setStatus('GET', me.filter.preview ? 200 : 401).setStatus('POST', 401).setStatus('DELETE', 401));
    else if (me.filter.userObjId.equals(me.operator.objId))
        return bluebird.resolve(httpOptions.setStatus('GET', 200).setStatus('POST', 200).setStatus('DELETE', 401));
    else {
        return bluebird.all([me.operator.isGod(), me.operator.isOb()])
        .then(function (data) {
            if (data[0])
                return httpOptions.setStatus('GET', 200).setStatus('POST', 200).setStatus('DELETE', 200);
            else if (data[1])
                return httpOptions.setStatus('GET', 200).setStatus('POST', 401).setStatus('DELETE', 401);
            else
                return httpOptions.setStatus('GET', me.filter.preview ? 200 : 401).setStatus('POST', 401).setStatus('DELETE', 401);
        });
    }
};
...
SingleUser.prototype._post = function () {
    var me = this;
    return me._accessibility()
    .then(function (httpOptions) {
    ...
    .then(function (currentUser) {
        ...
        if (validate.value(me.args.headUrl)) {
            if (validate.picBase64(me.args.headUrl))
                update.headUrl = me.args.headUrl;
            else if (validate.url(me.args.headUrl)) {
                var parsed = extension.string.parseExternalStorageUrl(me.args.headUrl);
                if (parsed)
                    update.headUrl = parsed;
                else
                    update.headUrl = { path: me.args.headUrl, storage: 'raw' };
            }
            else
                throw new errdef.DeferChainKiller(extension.http.webres400());
        }
        ...
        if (validate.value(me.args.personal)) {
            ...
            if (validate.value(me.args.personal.nickName)) {
                ...
                else if (validate.emptyString(me.args.personal.nickName))
            ...
            if (validate.value(me.args.personal.gender)) {
                ...
                else if (validate.emptyString(me.args.personal.gender))
        ...
        if (extension.obj.objLen(update) == 0)
            throw new errdef.DeferChainKiller(extension.http.webres400());
        return resxutil.user.checkDup(null, update['personal.nickName'], me.operator.objId)
        .then(function (ok) {
            if (ok)
