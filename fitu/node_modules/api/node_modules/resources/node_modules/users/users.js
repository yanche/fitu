...
Users.prototype._post = function () {
    ...
    .then(function (httpOptions) {
        ...
        if (status.isOK()) {
            if (!me.args)
                throw new errdef.DeferChainKiller(extension.http.webres400());
            var create = { special: 0, fans: [], subscribe: { users: [], sites: [] }, createdOn: new Date() };
            //email
            ...
            if (validate.value(me.args.personal)) {
                ...
                else if (!validate.nullOrEmptyStr(me.args.personal.nickName))
                    throw new errdef.DeferChainKiller(extension.http.webres400());
                //phone
                ...
                if (validate.gender(me.args.personal.gender))
                    ...
                    create.personal.gender = constants.common.gender.unknown;
            }
            else
                create.personal = { gender: constants.common.gender.unknown, contact: create.email };
            //headUrl
            if (validate.picBase64(me.args.headUrl))
                ...
                if (parsed)
                    create.headUrl = parsed;
                else
                    create.headUrl = { path: me.args.headUrl, storage: 'raw' };
            }
            else if (validate.nullOrEmptyStr(me.args.headUrl))
                create.headUrl = { path: userdefaultheadpic.getUserDefaultHeadPic(create.personal.gender), storage: 'local' };
            else
                throw new errdef.DeferChainKiller(extension.http.webres400());
    ...
    .then(function (inserted) {
        ...
        note.toSysNote({ emphasis: false, subject: '恭喜你注册成功啦！', subjectType: constants.note.contentTypes.plain, body: '在这里找到一群热爱运动的小伙伴吧！', bodyType: constants.note.contentTypes.plain, recipientObjId: inserted._id });
        //TODO: log if failed
