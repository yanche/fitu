...
WebPageService.prototype.handle = function (webreq) {
    if (webreq.pathname.indexOf('/app') == 0)
        ...
        return extension.http.webres200(constants.mime.html, cpl({
            ...
            storageBase: url.format(config.storageFileServer),
            apiBase: url.format(config.apiServer),
            domain: config.domain,
            mobile: validate.userAgentMobile(webreq._raw.headers['user-agent']),
            wechatCallback: config.wechatloginCallback
        }), { 'Cache-Control': config.env == 'prod' ? ('public, max-age=' + extension.datetime.getFileMaxAge()) : 'no-cache' });
    }
};
